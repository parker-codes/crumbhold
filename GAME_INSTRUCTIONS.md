# How to play Crumbhold

You are the Warden, a soldier ant. Below you is the Great Gallery, and at its
centre sits the queen and her egg clusters: the Brood Chamber.

Three tunnels lead in from the surface. Every night, something comes down them.

## The object

**Keep the Brood Chamber alive for twelve nights.**

That is the whole win condition. Your own health does not end the run — at zero
you curl up and two workers drag you home. Nothing else on the map matters if the
chamber falls, and nothing else matters if it holds.

Clear night twelve and you can keep going into an endless ladder on the same
gallery, with the raids still growing.

## The shape of a run

A run is twelve nights, roughly twelve to fifteen minutes. Each night is three
beats.

| Beat | Length | What happens |
| --- | --- | --- |
| **Day** | 45 seconds | No invaders. Gather, convert, and build. |
...
You carry everything physically, balanced over your head in up to two columns.
Sugar is always the left column; the right column shows whichever other resource
you picked up most recently. You can read how rich you are from across the
gallery, and so can anyone watching.

Anything on the floor within about 70 units leaps to you on its own — no pickup
button. Mounted, that reach grows to 110.

Three resources, one currency:

| Resource | Carry limit | Comes from | Goes to |
| --- | --- | --- | --- |
| **Sugar** | 25 | Fallen invaders, and conversions | Everything. The only spend currency. |
...
You never tap to buy. Every buildable sits on a pad on the floor. **Walk onto the
pad and stay there**, and sugar lifts off your stack into it at 14 per second.
...
Locked pads are visible from the very first day as faint dashed outlines with
their requirement written on them. That is deliberate: the whole build tree is
readable by walking around and looking at the floor. There is no tech tree menu
...
while mounted, so it is a hauling tool more than a fighting one. It cannot be
killed and never needs repairing.
...
The raid grows about 32% every night. Tunnels open on a schedule, so early
mistakes are cheap and late ones are not:

| Nights | Tunnels active |
| --- | --- |
| 1–2 | North only |
| 3–5 | North and southwest |
| 6–12 | All three |

Every creature is built to be told apart by **shape alone**, not colour, so you
can read a swarm at a glance.

| Invader | From | Health | Reads as | What it does to you |
| --- | --- | --- | --- | --- |
| **Raider Ant** | 1 | 30 | Thin, three clean segments | The baseline. Dangerous only in numbers. |
| **Bombardier Beetle** | 2 | 24 | Rounded dome, abdomen raised over its back | Sprays from 200 away. Fragile, but it will whittle a barricade from outside your posts' reach. |
| **Pillbug** | 3 | 70 | Wide, low, segmented oval | Acid hitting its **front does 60% less**, and it curls when you land one. Flank it, or run it down with the beetle — the horn ignores the armour. The only positional puzzle in the game. |
| **Exploding Termite** | 4 | 40 | Pale body, bulbous abdomen | Sprints in and detonates for 35 in a wide blast. Kill it early; never let a group reach anything at once. |
| **Mole Cricket** | 5 | 150 | Broad shovel forelimbs | **Ignores you completely.** It only ever attacks structures, and it hits them for 40. This is the creature barricades exist for. |
| **Wolf Spider** | 6 and 12 | 1200+ | Eight legs, twice everything else's footprint | The boss. Summons three spiderlings at 60% health. Arrives on nights 6 and 12 only, and scales hard the second time. |
| **Tiger Beetle** | 7 | 90 | Long, low, narrow | Moves in stop-start sprints, so it closes distance in bursts you cannot walk away from. |
| **Stag Beetle** | 8 | 220 | Huge branching mandibles | Slow, tough, and knocks you backwards. Bad to be cornered by. |

Each night reserves a fifth of its strength for whichever creature is newest, so
the thing you have not seen before always shows up in numbers worth noticing.

## Strategy

**Buy a Soldier Gallery early.** 45 sugar, hatches free every night for the rest
of the run. Nothing else in the game compounds like that.

**Work the economy, not just the corpses.** Conversions are worth roughly a
thousand sugar over twelve nights — a fifth to a third of your total income, and
a floor rather than a ceiling, since the pens and roots produce faster than a
distracted player collects. Ignoring them is ignoring a Brood Chamber's worth of
upgrades.
...
arrives every night that ignores you entirely and eats your buildings. A barricade
...
- **The Surface Band** across the top is the whole clock. A sun crosses it by day
  and a moon by night, and the twelve brood cells set into the soil fill amber as
  you hold each night. It is your progress bar, phase indicator, and the source of
  the daylight shaft below.
- **Top left** is what you are carrying. A `/25` appears once you are near full.
- **Under the band, at night**, the Brood Chamber's health and how many waves are
  left.
- **Chevrons on the screen edge** point at invaders you cannot see, coloured by
  how dangerous they are.
- **On the floor**: every cost, every fill ring, every lock requirement. The room
  tells you everything; that is why there are almost no menus.
